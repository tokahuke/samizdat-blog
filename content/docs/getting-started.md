...
Ok... it is not your run-of-the-mill site name, such as
...
in this computer" (since it is where Samizdat node is running, after all) and
...
as `mail.google.com`. It is the _name_ of this computer. The _route_, `/mail`
...
<aside class="note">
Heads up: these <code>localhost:4510</code> examples only work if you have a Samizdat node running with the content cached. From the regular Web, they 404. The point here is to show how the URLs are <em>shaped</em>, not to make them clickable.
</aside>
...
2. You cannot have cyclic links! That is, if page A has a link to page B, page B cannot have a link to page A. Imagine not being able to link back from your blog post to your homepage!
...
Just go surf the Web as you normally do and SAMIZDAT will take care of the rest for you! But remember: things inside the network are referenced by _what_ they are (content addressing) and not by _where_ they are (location addressing). Even that is not that big of a deal, since the original Web experience was already created to give you this illusion. 
